...
var _ = Describe("Queue Validator", func() {
	...
	Context("resource value validation (quota validation enabled)", func() {
		...
		It("warns when a limit is set below its quota", func() {
			...
			warnings, err := validator.ValidateCreate(ctx, queue)
			Expect(err).NotTo(HaveOccurred())
			Expect(warnings).To(ContainElement(ContainSubstring("CPU limit (1000.00) is below its quota (2000.00)")))
			Expect(warnings).To(ContainElement(ContainSubstring("GPU limit (2.00) is below its quota (4.00)")))
			Expect(warnings).To(ContainElement(ContainSubstring("Memory limit (4096.00) is below its quota (8192.00)")))
		})
		...
		It("warns on invalid negative values", func() {
			...
			warnings, err := validator.ValidateCreate(ctx, queue)
			Expect(err).NotTo(HaveOccurred())
			Expect(warnings).To(ContainElement(ContainSubstring("CPU quota (-5.00) is invalid")))
			Expect(warnings).To(ContainElement(ContainSubstring("GPU limit (-5.00) is invalid")))
			Expect(warnings).To(ContainElement(ContainSubstring("Memory overQuotaWeight (-1.00) is invalid")))
		})
	...
	Context("parent-child quota validation (quota validation enabled)", func() {
		...
		It("warns when the sum of children GPU quota exceeds the parent", func() {
			...
			Expect(err).NotTo(HaveOccurred())
			Expect(warnings).To(ContainElement(ContainSubstring("total children GPU quota (3.00) exceeds parent queue parent-queue GPU quota (2.00)")))
		})
