...
func TestDefaultConfig(t *testing.T) {
	...
	if got := config[GPUSharingPluginName].Arguments[HamiCoreEnabledArgument]; got != "true" {
		t.Fatalf("expected gpusharing hami core true, got %q", got)
	}
...
func TestArgumentParsers(t *testing.T) {
	...
	if got, err := boolArgumentOrDefault(map[string]string{}, HamiCoreEnabledArgument, DefaultHamiCoreEnabled); err != nil || !got {
		t.Fatalf("expected missing hami core argument to use default true, got %t, err %v", got, err)
	}
