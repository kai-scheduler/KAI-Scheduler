...
func getResourceSlices(testMetadata TestTopologyBasic) []*resourceapi.ResourceSlice {
	poolSliceCount := map[string]int64{}
	for _, rs := range testMetadata.ResourceSlices {
		poolSliceCount[rs.NodeName]++
	}
		...
		resourceSliceObject := resourceapi.ResourceSlice{
			...
			Spec: resourceapi.ResourceSliceSpec{
				...
				Pool: resourceapi.ResourcePool{
					...
					ResourceSliceCount: poolSliceCount[resourceSlice.NodeName],
				},
