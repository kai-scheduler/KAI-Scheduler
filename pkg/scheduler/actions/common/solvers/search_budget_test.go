...
func TestActionSearchBudgetDefaults(t *testing.T) {
	...
	require.Equal(t, 30*time.Second, budget.generatorLimits[scenariosearch.GeneratorNodeLocalGreedy])
	require.Equal(t, 2*time.Minute, budget.generatorLimits[scenariosearch.GeneratorMultiNodeGang])
	require.Equal(t, 2*time.Minute, budget.generatorLimits[scenariosearch.ActionDefault])
}
