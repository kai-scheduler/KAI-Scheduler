...
func containerUnits(pod *v1.Pod) []resourceAmounts {
	units := make([]resourceAmounts, 0, len(pod.Spec.Containers)+len(pod.Spec.InitContainers))
...
func toAmounts(list v1.ResourceList) resourceAmounts {
	amounts := make(resourceAmounts, len(list))
