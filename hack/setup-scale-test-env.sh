...
helm upgrade -i fake-gpu-operator oci://ghcr.io/run-ai/fake-gpu-operator/fake-gpu-operator --namespace gpu-operator --create-namespace -f "${SCRIPT_DIR}/fake-gpu-operator-values.yaml" --set topology.nodePools.default.gpuCount=8 --devel --version 0.2.0
