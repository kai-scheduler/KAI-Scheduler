...
1. Pod `STATUS: SchedulingGated` (kubectl shows it directly), or `spec.schedulingGates` non-empty.
...
2. No pods at all + the owner is a `Job` with `spec.suspend: true` -> suspended (e.g. by Kueue);
