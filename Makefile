...
changelog-release: changie ## Fold unreleased fragments into CHANGELOG.md as VERSION and clear them. Usage: make changelog-release VERSION=v0.16.5
	@test -n "$(VERSION)" || { echo "VERSION is required, e.g. make changelog-release VERSION=v0.16.5"; exit 1; }
	CHANGIE=$(CHANGIE) bash hack/changelog-fold.sh $(VERSION)

.PHONY: changelog-preview
changelog-preview: changie ## Preview the next release section without writing anything. Usage: make changelog-preview VERSION=v0.16.5
	@test -n "$(VERSION)" || { echo "VERSION is required, e.g. make changelog-preview VERSION=v0.16.5"; exit 1; }
