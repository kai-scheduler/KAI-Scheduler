...
| Deferred resize preemption | Separate track: [#1872](https://github.com/kai-scheduler/KAI-Scheduler/issues/1872), [deferred-resize-eviction.md](deferred-resize-eviction.md). Depends on `Deferred` charged at max(spec, actual) — the reserved target is its thrash safety |
