...
| `gpusharing` | 100 | `cdiEnabled: "false"`, `hamiCoreEnabled: "true"` | Handles fractional GPU pod mutation needed for GPU sharing. |

For operator-managed deployments, the operator sets the `gpusharing` `cdiEnabled` argument from `spec.binder.cdiEnabled`. If `spec.binder.cdiEnabled` is unset, the operator attempts to auto-detect CDI from the NVIDIA GPU Operator `ClusterPolicy`. The `hamiCoreEnabled` argument defaults to `true`.
...
When `hamiCoreEnabled` is `true`, the plugin also writes `CUDA_DEVICE_MEMORY_LIMIT` to the GPU sharing ConfigMap based on node label `nvidia.com/gpu.memory` and `BindRequest.spec.receivedGPU.portion`.
