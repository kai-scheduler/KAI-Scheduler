...
| `total_preemption_attempts` | Counter | `endpoint`, `instance`, `job`, `namespace`, `pod`, `service` | **Deprecated.** Counts successful preemptions, not attempts, despite the name. Use `podgroups_scheduled_by_action{action="preempt"}` for successes and `podgroups_acted_on_by_action{action="preempt"}` for attempts. |
| `preemption_failures_total` | Counter | `endpoint`, `instance`, `job`, `namespace`, `pod`, `service`, `reason` | Cumulative count of preemption attempts that ended without preempting, by terminal reason. Together with `podgroups_scheduled_by_action{action="preempt"}` this accounts for every attempt counted by `podgroups_acted_on_by_action{action="preempt"}`. Attempts that reached scenario search are also broken down by `scenario_search_jobs_total{action="preempt"}`; `reason="preemptor_over_quota"` covers the attempts that never enter it. |
