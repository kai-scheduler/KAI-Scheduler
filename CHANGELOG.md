...
### Fixed
- The PodGroup and Queue status now count native sidecars (`initContainers` with `restartPolicy: Always`) and Pod `Overhead`; only regular containers were summed before, so everything a sidecar requests and every non-GPU overhead entry was missing from `PodGroup.status.resourcesStatus`, `Queue.status.allocated` and the `kai_queue_allocated_*` metrics. The peak of a non-restartable init container is still not counted, so for those pods the status stays below what the scheduler reserves. `kai_queue_allocated_*` feeds the scheduler's time-based fairshare usage database by default, so where that is enabled these metrics step up once on upgrade for pods that are already running. [#1880](https://github.com/kai-scheduler/KAI-Scheduler/issues/1880) [thc1006](https://github.com/thc1006)
