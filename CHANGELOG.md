...
- Reuse the solution result produced by `JobSolver.searchMaxSolvableK` when the doubling phase reaches `k == n`, eliminating the redundant `probeAtK(n)` re-simulation that previously ran on the success path. Builds on [#1435](https://github.com/kai-scheduler/KAI-Scheduler/pull/1435).
