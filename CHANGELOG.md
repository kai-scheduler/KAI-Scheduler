...
### Fixed
- The PodGroup and Queue status now count native sidecars (`initContainers` with `restartPolicy: Always`) and Pod `Overhead`; only regular containers were summed before, so a sidecar's requests and a RuntimeClass overhead were missing from `PodGroup.status.resourcesStatus` and from `Queue.status.allocated`, `allocatedNonPreemptible` and `requested`. `kai_queue_allocated_cpu_cores` and `kai_queue_allocated_memory_bytes` rise accordingly for queues running sidecar or sandboxed workloads, and since they are the default input to the scheduler's time-based fairshare usage database they step once on upgrade for pods that are already running. Two things are still left out, both tracked in #1880: an NVIDIA or AMD GPU, or a MIG device, asked for by a sidecar or set in an overhead, which needs the scheduler's GPU annotation handling mirrored first; and the peak of a non-restartable init container, which needs the max semantics the scheduler applies. [#1880](https://github.com/kai-scheduler/KAI-Scheduler/issues/1880) [thc1006](https://github.com/thc1006)
