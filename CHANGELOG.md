...
### Added
- Made a kubelet-deferred in-place pod resize (KEP-1287) a scheduling event KAI acts on, with no new action, config, CRD, or Helm change: when the kubelet defers a resize for lack of capacity (`PodResizePending` condition, reason `Deferred`), KAI represents the growth as a synthetic pending demand pinned to the resizing pod's **own node** and lets its existing `allocate`/`preempt`/`reclaim` free room for it, so the kubelet can actuate the resize on its next attempt — KAI never calls the `/resize` subresource itself. Because the growth is scheduled exactly like any other pending pod in the queue, KAI's existing preemption and reclaim rules apply to it unchanged (no resize-specific gate): it preempts only strictly-lower-priority same-queue work and reclaims from other queues only within fair share, and is left `Deferred` when the queue is not entitled — just as such a pending pod would stay `Pending`. The resizing pod is charged at its actual (granted) size with the growth reserved separately and never bound, so the delta is not double-counted and the freed space cannot be taken by a lower-priority pod. If no eligible victim exists the resize is left deferred, causing no disruption; `Infeasible` resizes are ignored since freeing capacity cannot help them ([design](docs/developer/designs/deferred-resize-preemption/README.md), [example](examples/deferred-resize-preemption/README.md)). [#1872](https://github.com/kai-scheduler/KAI-Scheduler/issues/1872)
