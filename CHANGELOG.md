...
### Fixed
- Fixed a PodGroup's `UnschedulableOnNodePool` scheduling condition persisting after the workload schedules: `RecordJobStatusEvent` now clears the condition for the node pool once the group's tasks are bound, so status consumers no longer see a running workload as unschedulable. The condition is kept while any task is still awaiting bind, so a failed bind still leaves an explanation for why the pods were pending. [#1888](https://github.com/kai-scheduler/KAI-Scheduler/issues/1888) [Thezone-1](https://github.com/Thezone-1)
