...
### Fixed
- Fixed a PodGroup's `UnschedulableOnNodePool` scheduling condition persisting after the workload schedules: `RecordJobStatusEvent` now clears the condition for the node pool once the group has no pending or gated tasks, so status consumers no longer see a running workload as unschedulable. [#1888](https://github.com/kai-scheduler/KAI-Scheduler/issues/1888) [Thezone-1](https://github.com/Thezone-1)
