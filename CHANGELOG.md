...
- Added an opt-in `--strict-queue-validation` flag to the queue-controller. When enabled, queue updates that reduce a resource limit below the amount currently allocated to the queue, or reduce a quota below the amount allocated to non-preemptible workloads (both read from the queue status), are rejected. When only `--enable-quota-validation` is set, the same conditions are reported as admission warnings rather than blocking. [#1783](https://github.com/kai-scheduler/KAI-Scheduler/issues/1783) [thc1006](https://github.com/thc1006)
