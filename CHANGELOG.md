...
- The scheduler's per-shard Service is now populated by an operator-managed `EndpointSlice` pointing at the current leader-election Lease holder, which is connected to the service of the shard's scheduler. This allows the service to route all it's incoming request to the lease-holding pod of the scheduler deployment.

### Fixed
- Added scheduler liveness/readiness probes and made replicated scheduler shards report available when one updated leader pod is ready, so Services avoid standby schedulers while preserving active-passive HA status. [#1484](https://github.com/kai-scheduler/KAI-Scheduler/issues/1484)
