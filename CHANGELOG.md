...
- Added a `semi-preemptible` PodGroup preemptibility mode. A job is non-preemptible up to its **minimal satisfying set** — the tree's core, computed recursively as the `minSubGroup` highest-priority child subgroups at each level and `minMember` pods at each leaf PodSet — while anything beyond that minimum is elastic and preemptible, letting workloads guarantee a minimum allocation while bursting opportunistically. The core/elastic split applies at every level of the subgroup tree, so it composes with segmented subgroups: surplus subgroups (e.g. whole segments) are reclaimed atomically. Only core resources count toward queue quota (`AllocatedNotPreemptible`), reclaim/preempt evict only the elastic surplus (the core is never offered as a victim), and `minMember`/`minSubGroup` are immutable (cannot be increased) on semi-preemptible PodGroups. [#1713](https://github.com/kai-scheduler/KAI-Scheduler/pull/1713)
- The podgrouper now emits a `PodGrouperWarning` event when a workload is both automatically segmented (`kai.scheduler/segment-size`) and `semi-preemptible`. The two are mutually exclusive — a segmented workload is fully gang and has no elastic surplus, so semi-preemptible has no effect and the workload behaves as non-preemptible. The PodGroup is still created (soft enforcement); use semi-preemptible with elastic workloads or hand-authored `minSubGroup` SubGroup trees instead.
