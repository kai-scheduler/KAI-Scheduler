...
### Fixed
- PodGroup and Queue status now count native sidecars (`initContainers` with `restartPolicy: Always`) and Pod `Overhead`, not only regular containers. A queue running Istio-style sidecars, or a sandboxed runtime such as Kata or gVisor, was reporting less than its pods actually held for as long as they ran. On upgrade `kai_queue_allocated_cpu_cores` and `kai_queue_allocated_memory_bytes` step up once for those queues, which also shifts the scheduler's time-based fairshare usage database where it reads them; `kai_queue_allocated_gpus` is unchanged. [Queue Status](docs/queues/README.md#queue-status) describes what is counted and what is still left out. [#1880](https://github.com/kai-scheduler/KAI-Scheduler/issues/1880) [thc1006](https://github.com/thc1006)
