...
### Fixed
- The PodGroup and Queue status now count native sidecars (`initContainers` with `restartPolicy: Always`) and Pod `Overhead`; only regular containers were summed before, so a sidecar's CPU, memory and other non-GPU requests, and the non-GPU part of a RuntimeClass overhead, were missing from `PodGroup.status.resourcesStatus` and `Queue.status.allocated`. `kai_queue_allocated_cpu_cores` and `kai_queue_allocated_memory_bytes` rise accordingly for queues running sidecar or sandboxed workloads, and since they are the default input to the scheduler's time-based fairshare usage database, they step once on upgrade for pods that are already running. A sidecar's GPU and the peak of a non-restartable init container are still not counted; both need the scheduler's GPU annotation handling and are tracked in #1880. [#1880](https://github.com/kai-scheduler/KAI-Scheduler/issues/1880) [thc1006](https://github.com/thc1006)
