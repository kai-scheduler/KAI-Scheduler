...
- Added optional HAMI-core GPU memory limit support (`CUDA_DEVICE_MEMORY_LIMIT`) to the `gpusharing` binder plugin via a new `hamiCoreEnabled` argument. Disabled by default; opt in by setting `hamiCoreEnabled: "true"`. [#1504](https://github.com/kai-scheduler/KAI-Scheduler/pull/1504)
