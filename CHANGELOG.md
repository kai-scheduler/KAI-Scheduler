...
- Skip a redundant final solver probe in `JobSolver.Solve` when `searchMaxSolvableK` has already determined that no full allocation is feasible, trimming one scenario simulation per unschedulable job. Builds on [#1435](https://github.com/kai-scheduler/KAI-Scheduler/pull/1435).
