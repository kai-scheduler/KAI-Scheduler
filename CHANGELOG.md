...
- Reduced scheduler memory use for large unschedulable workloads by retaining task fit-error reason counts instead of per-node details; detailed node errors are recomputed only when requested. [#1827](https://github.com/kai-scheduler/KAI-Scheduler/issues/1827)
- Reduced scheduler memory use for large unschedulable workloads by retaining task fit-error reason counts instead of per-node details, recomputing detailed node errors only when requested, and publishing diagnostics only from authoritative allocation attempts. [#1827](https://github.com/kai-scheduler/KAI-Scheduler/issues/1827)
